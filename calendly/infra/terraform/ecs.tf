...
# Backend (Rails 8 + Ruby 4 / Solid Queue を同 task に同居 / SOLID_QUEUE_IN_PUMA=1)。
# スループット要求が上がったら別 service に分離するのが次の一手 (ADR 0002)。
resource "aws_ecs_task_definition" "backend" {
  family                   = "calendly-backend"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "DB_USER", value = "calendly_admin" },
        { name = "AI_WORKER_URL", value = "http://ai-worker.calendly.local:8090" },
  ...
  container_definitions = jsonencode([
    {
      ...
      essential    = true
      portMappings = [{ containerPort = 3105 }]
      environment = [
