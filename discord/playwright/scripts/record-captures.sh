...
#
# 使い方:
#   cd discord/playwright && npm run capture
#   PLAYBACK_RATE=2.5 npm run capture
#
# 必要: ffmpeg + 全サービス起動 (mysql / Go gateway WS / ai-worker / Next)
...
capture_name_for() {
  case "$1" in
    *WebSocket*fan-out*) echo "01-websocket-fanout" ;;
    *presence*offline*)  echo "02-presence-offline-on-tab-close" ;;
