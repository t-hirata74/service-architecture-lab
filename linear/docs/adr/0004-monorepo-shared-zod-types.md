...
TypeScript フルスタックの最大の利点は「FE と BE が同じ型で同じプロトコルを喋れる」ことにある。本プロジェクトでは sync protocol (mutation コマンド / op payload / WS メッセージ / bootstrap・delta レスポンス) が FE/BE のコントラクトそのものであり、これが二重定義されると optimistic 適用 (ADR 0003) と server 適用の意味がズレて収束が壊れる。
...
- スキーマが**ランタイム検証と型の両方**を担い、コントラクトが 1 ファイルに見える
...
- REST コントラクトの業界標準。他言語クライアントにも開ける
...
- **学習価値**: 「スキーマ駆動のコントラクト共有」という TS フルスタック固有の設計を、ライブラリに隠されず素の構成で組む。reducer 共有により「楽観適用と server 適用の意味の一致」という sync engine の核心が型で表現される
- **アーキテクチャ妥当性**: zod をコントラクトの単一源泉にする構成は実務の TS モノレポで標準的。tRPC/codegen との比較判断自体が実務でよく問われる
...
- **OpenAPI ドキュメントを持たない**: slack/youtube のような openapi-lint ジョブは作れない。コントラクトの可読性は shared/ のスキーマ定義自体で担保する
...
- `linear/shared/src/schema/` — entity / mutation / protocol / ai スキーマの単一定義
- `linear/backend/src/common/zod-validation.pipe.ts` — shared スキーマでの入力検証
- `linear/backend/test/reducer-parity.e2e-spec.ts` — **bootstrap(0) + 全 ops の reducer 畳み込み ≡ 最終 bootstrap** を実 DB で突き合わせる parity テスト (Phase 4 で実装・pass)
- `linear/shared/src/default-states.ts` — backend の seed と client の楽観適用が同じ定義を参照する単一源泉の例
