...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DB_USERNAME", value = "github" },
      # Redis なし (ADR 0001: Solid Queue + Solid Cache + Solid Cable で DB 駆動)
      { name = "AI_WORKER_URL", value = "http://ai-worker.github.internal:8000" },
