...
#
# 使い方:
#   cd youtube/playwright && npm run capture
#   PLAYBACK_RATE=2.5 npm run capture
#
# 必要: ffmpeg + 全サービス起動 (mysql / Rails / ai-worker / Next + Solid Queue)
...
capture_name_for() {
  case "$1" in
    *browse*home*detail*) echo "01-browse-feed-and-detail" ;;
    *upload*state*)       echo "02-upload-state-machine" ;;
    *search*results*)     echo "03-search-fulltext-hit" ;;
    *search*empty*non-matching*) echo "04-search-no-match" ;;
