...
> 用語: 本リポで **API コントラクト**(または単にコントラクト)と呼ぶのは「FE/BE 間でリクエスト/レスポンスやメッセージの形をこう取り決める」という合意のこと。それが守られているかを自動検証するテストが **API コントラクトテスト**(例: openapi-lint、zod パース検証)。
...
| --- | --- | --- |
| **FE/BE のコントラクトそのものが設計の主役** (protocol / コマンド / スキーマ) | linear の sync protocol (mutation / op / bootstrap / WS message) | OpenAPI codegen でも型は渡せるが、**zod なら 1 定義 = 型 + 両側のランタイム検証**になり、生成ステップも消える |
...
### React (Next.js) + Rails 構成との比較

本リポの実測対比: **React + Rails API は 8 本** (slack / youtube / github / perplexity / shopify / zoom / calendly / figma)、**TS フルスタックは 1 本** (linear)。どちらも frontend は Next.js なので、差は **「backend を Rails にするか Node (NestJS) にするか」= コントラクトとロジックを言語境界で切るか共有するか** に集約される。

| 観点 | React (Next.js) + Rails API | TS フルスタック (Next.js + NestJS) |
| --- | --- | --- |
| **FE/BE のコントラクト** | OpenAPI / GraphQL schema を境界に置き codegen or 手書き型。**二重定義 + ドリフトはAPI コントラクトテストで抑える** (slack/youtube の openapi-lint, github の schema diff) | zod 1 定義 = 型 + 両側ランタイム検証。**codegen ステップ自体が消える**。意味の一致は parity テストまで踏み込める (linear) |
| **ドメインロジックの置き場所** | server に集約。client は表示 + 入力検証の薄い複製 (二重実装は最小限に抑える規律が要る) | **client でも同じロジックが動く** (楽観適用 reducer / fractional indexing)。optimistic・offline・エディタ系で決定的な差になる |
| **CRUD / 管理画面 / フォーム** | **Rails 圧勝** — generator / AR / strong params / コンソール / gem 生態系。本リポ 8 本が Rails なのはドメイン CRUD + 状態機械 + 権限が主役だったから | Prisma + Nest はボイラープレートが多く、AR 相当の速度は出ない |
| **非同期ジョブ / キュー** | Solid Queue / Sidekiq が「標準」として成熟 (youtube / shopify / zoom) | BullMQ 等はあるが Rails ほどの規約・一体感はない。ジョブ基盤が重い題材は Rails に倒す |
| **リアルタイム** | ActionCable + Solid Cable/Redis — 手軽だが抽象の内側 (slack / figma) | 素の ws を直接制御 — 再接続・heartbeat・room を自分で設計する (linear ADR 0005)。制御性と学習量はこちらが上 |
| **型安全** | Ruby は Sorbet/RBS が後付けで、FE 型との接続はコントラクト経由のみ | **end-to-end で 1 つの型システム**。リファクタが FE/BE を跨いで追従する |
| **頭の切替コスト** | Ruby ⇄ TS の言語切替 + validation 等の規則二重持ち | 1 言語。FE/BE を同じ人が触る小チームで効く |
| **テスト** | RSpec 生態系 (request spec / FactoryBot) が厚い | jest + vitest 併用 (§7)。**parity テスト**という固有手法が使える |

### 判断フロー (React + Rails か、TS フルスタックか)

1. **ドメインロジックは server 側だけで完結するか?** — Yes なら React + Rails で十分。コントラクトは OpenAPI / GraphQL + API コントラクトテストで守る (本リポ 8 本の型)
2. **client でも同じドメインロジックを動かす必要があるか?** (optimistic / offline / リッチエディタ / client 側計算) — Yes なら TS フルスタックが第一候補。Rails で作ると JS 側に「劣化コピー」が必ず生まれ、ドリフトを止める仕組みがない
3. **コントラクトの変更頻度が高く、FE/BE を同じ人が往復するか?** — Yes なら monorepo 型共有の DX (リファクタ追従・codegen レス) が日々効く
4. **CRUD・管理画面・ジョブ基盤の比重が大きいか?** — Yes なら Rails に倒すか、**ハイブリッド** (Rails をドメイン API に据え、Node は streaming BFF / 共有ロジック部分だけ担う) を検討する。「全部 TS に寄せる」は CRUD の生産性を捨てる対価を払う

> 要するに: **境界のコントラクトを「守る」のが主題なら Rails + API コントラクトテスト、境界を「消す」のが主題なら TS フルスタック**。linear は 2 が題材そのものだったので TS、他 8 本は 1 と 4 が支配的だったので Rails、という整理になる。

### 本リポでの採用例

- **linear (MVP 完成)** — server 権威 sync log ⇄ optimistic/offline client。「FE/BE が同じコントラクト・同じ reducer を共有しないと成立しない」題材で、TS 固有価値が最大化される。Rails (8 本) との対比で「Node 版 Rails としての NestJS」も検証した (§9)
...
- **コントラクトは zod 単一定義** — スキーマが「TS 型 (`z.infer`) + backend 入力検証 + frontend フォーム/受信検証」の 3 役を兼ねる。tRPC は HTTP の形がランタイムに隠れ WS と二重化する、OpenAPI codegen は WS をカバーできず生成物管理が増える、という判断 (linear ADR 0004)
...
- **validation は class-validator でなく zod pipe**: `@Body(new ZodValidationPipe(SchemaFromShared))`。Nest 公式の慣習から外れる代わりに、コントラクトの単一定義 (§2) を取る
