...
本リポの実測対比: **React + Rails API は 8 本** (slack / youtube / github / perplexity / shopify / zoom / calendly / figma)、**TS フルスタックは 1 本** (linear)。どちらも frontend は Next.js なので、差は **「backend を Rails にするか Node (NestJS) にするか」= 契約とロジックを言語境界で切るか共有するか** に集約される。

| 観点 | React (Next.js) + Rails API | TS フルスタック (Next.js + NestJS) |
| --- | --- | --- |
| **FE/BE の契約** | OpenAPI / GraphQL schema を境界に置き codegen or 手書き型。**二重定義 + ドリフトは契約テストで抑える** (slack/youtube の openapi-lint, github の schema diff) | zod 1 定義 = 型 + 両側ランタイム検証。**codegen ステップ自体が消える**。意味の一致は parity テストまで踏み込める (linear) |
...
| **型安全** | Ruby は Sorbet/RBS が後付けで、FE 型との接続は契約経由のみ | **end-to-end で 1 つの型システム**。リファクタが FE/BE を跨いで追従する |
...
1. **ドメインロジックは server 側だけで完結するか?** — Yes なら React + Rails で十分。契約は OpenAPI / GraphQL + 契約テストで守る (本リポ 8 本の型)
...
3. **契約の変更頻度が高く、FE/BE を同じ人が往復するか?** — Yes なら monorepo 型共有の DX (リファクタ追従・codegen レス) が日々効く
...
> 要するに: **境界の契約を「守る」のが主題なら Rails + 契約テスト、境界を「消す」のが主題なら TS フルスタック**。linear は 2 が題材そのものだったので TS、他 8 本は 1 と 4 が支配的だったので Rails、という整理になる。
