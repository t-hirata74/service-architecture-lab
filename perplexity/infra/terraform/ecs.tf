################################
# ECS Cluster + 3 Services (Fargate)
# - frontend (Next.js)  port 3000
# - backend  (Rails 8 + ActionController::Live) port 3000
# - ai-worker (FastAPI + numpy hybrid retrieval) port 8000
...
# Service Discovery (backend → ai-worker への内部呼び出し用)
resource "aws_service_discovery_private_dns_namespace" "internal" {
  name = "perplexity.internal"
  vpc  = aws_vpc.main.id
}
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DB_USERNAME", value = "perplexity" },
      { name = "AI_WORKER_URL", value = "http://ai-worker.perplexity.internal:8000" },
    ...
    secrets = [
      ...
      { name = "RODAUTH_JWT_SECRET", valueFrom = aws_secretsmanager_secret.rodauth_jwt_secret.arn },
    ]
    logConfiguration = {
      logDriver = "awslogs"
      options = {
...
resource "aws_ecs_task_definition" "ai_worker" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "DB_HOST", value = aws_rds_cluster.main.reader_endpoint },
      { name = "DB_PORT", value = "3306" },
      { name = "DB_USERNAME", value = "perplexity_ro" },
      # ADR 0002 本番想定: hybrid retrieval を OpenSearch に切替える時の接続先.
      # ローカル (numpy in-memory) 構成では不要だが、本番化時は ai-worker が OpenSearch を呼ぶ.
