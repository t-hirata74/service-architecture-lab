...
variable "domain_name" {
  ...
  type        = string
  default     = "perplexity.example.com"
}
...
variable "frontend_image" {
  ...
  type        = string
  default     = "ghcr.io/example/perplexity-frontend:latest"
}

variable "backend_image" {
  description = "Rails backend コンテナイメージ"
  type        = string
  default     = "ghcr.io/example/perplexity-backend:latest"
}

variable "ai_worker_image" {
  description = "ai-worker (FastAPI) コンテナイメージ"
  type        = string
  default     = "ghcr.io/example/perplexity-ai-worker:latest"
}
...
variable "opensearch_instance_type" {
  description = "OpenSearch ドメインの data node インスタンスタイプ"
  type        = string
  default     = "r6g.large.search"
}

variable "opensearch_instance_count" {
  description = "OpenSearch data node 数 (3-AZ で 3 推奨)"
  type        = number
  default     = 3
}
