...
7. **認証 (rodauth-rails JWT bearer)** — accounts と users を共有 PK で 1:1 紐付け、`Authorization` ヘッダで SSE 含む全エンドポイントを保護 ([ADR 0007](adr/0007-auth-rodauth-jwt-bearer.md))
...
| テーブル | 役割 |
| --- | --- |
| `accounts` | rodauth-rails が管理。`email` (unique) / `status` (enum) / `password_hash` を持つ ([ADR 0007](adr/0007-auth-rodauth-jwt-bearer.md)) |
| `users` | アプリのドメイン。`accounts.id` と **共有 PK** で 1:1 紐付き、`after_create_account` フックで揃える |
...
| メソッド | パス | 用途 |
| --- | --- | --- |
| `POST` | `/create-account` | rodauth-rails: アカウント作成. `after_create_account` で同 id の User も作成 (ADR 0007) |
| `POST` | `/login` | rodauth-rails: 認証成功時に `Authorization` レスポンスヘッダで JWT を返す |
| `POST` | `/logout` | rodauth-rails: JWT 無効化 (本実装ではクライアント側 token 破棄を想定) |
...
- 認証導線 (Phase 5 で確定): **rodauth-rails の JWT bearer** を採用 ([ADR 0007](adr/0007-auth-rodauth-jwt-bearer.md))。`POST /login` 成功時に `Authorization` レスポンスヘッダで JWT を返却、以降は `Authorization` リクエストヘッダで認証。SSE エンドポイントも fetch + ReadableStream で同じヘッダを送れるため一貫
- トランジション期 (login UI 未実装の間) は `Authorization` 無しなら `X-User-Id` で fallback (development / test 専用 / production は 401)。完全移行後は fallback を削除
...
| 4 | SSE streaming endpoint + 引用整合性検証 + Frontend (fetch ReadableStream + 引用ハイライト UI). 認証は Phase 5 に繰り越し | 🟢 完了 |
| 5 | rodauth-rails JWT bearer (ADR 0007) + Playwright E2E + Terraform 設計図 + CI workflows | 🟢 完了 |
