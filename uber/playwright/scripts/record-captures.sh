...
# `captures/<test-name>.gif` に置く。2 BrowserContext のテストは hstack で
# driver | rider を横並びにして 1 つの gif にする (slack / discord と同じ仕組み)。
#
# 使い方:
#   cd uber/playwright && npm run capture
#   PLAYBACK_RATE=2.0 npm run capture
#
# 必要: ffmpeg + 全サービス起動 (mysql / Go dispatch + WS / ai-worker / Next)
...
capture_name_for() {
  case "$1" in
    *ride*dispatch*)  echo "01-dispatch-offer-accept" ;;
    *rider*cancel*)   echo "02-rider-cancel-matching" ;;
