// Package main は `go run ./cmd/migrate` で migrations/*.up.sql を順に適用する
// シンプルなマイグレーション runner。discord backend の同名プログラムと同形。
//
// 適用済 version は MySQL の `schema_migrations` テーブルで管理し、再実行で重複 apply しない。
package main
