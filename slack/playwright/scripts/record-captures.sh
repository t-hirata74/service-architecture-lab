...
#
# 使い方:
#   cd slack/playwright && npm run capture
#   PLAYBACK_RATE=2.5 npm run capture
#
# 必要: ffmpeg + 全サービス起動 (mysql:3306 / redis:6379 / Rails / ai-worker / Next)
...
capture_name_for() {
  case "$1" in
    *realtime-fanout*)        echo "01-realtime-fanout" ;;
    *read-sync*)              echo "02-read-cursor-auto-mark" ;;
    *summary*)                echo "03-ai-summary" ;;
    *auth*logout*login*)      echo "04-auth-signup-logout-login" ;;
    *auth*signup*)            echo "04-auth-signup" ;;
    *auth*未認証*)             echo "05-auth-unauthenticated-redirect" ;;
...
for dir in "$RESULTS_DIR"/*/; do
  ...
  # default fixture の `video.webm` が最優先。なければ multi-context tests の
  # `page@<hash>.webm` を使う。2 つ以上あれば hstack で side-by-side。
  webms=()
