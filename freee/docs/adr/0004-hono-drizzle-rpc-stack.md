...
- `shared/`（予定）— zod スキーマ（金額 NUMERIC 型含む）と RPC で共有する型の単一定義
- `backend/src/app.ts`（予定）— `AppType` の export 起点
- `frontend/src/api.ts`（予定）— `hc<AppType>` クライアント生成
- `backend/src/middleware/tenant.ts`（予定）— Drizzle transaction + `SET LOCAL`（ADR 0001 の実体）
