...
DB は MySQL 8（ADR 0003）。タイムゾーンは Rails アプリ層で UTC 統一する運用。

```text
messages
  id                BIGINT AUTO_INCREMENT (チャンネル横断で単調増加)
  channel_id        BIGINT (FK)
  user_id           BIGINT (FK)
  parent_message_id BIGINT (FK, nullable, スレッド用)
  body              TEXT
  created_at        DATETIME(6)
  edited_at         DATETIME(6) (nullable)
  deleted_at        DATETIME(6) (nullable, ソフトデリート)
  INDEX (channel_id, id)         -- チャンネル内タイムライン取得用
  INDEX (parent_message_id)      -- スレッド取得用
```

物理パーティションは取らない。学習目的としては「将来的に channel_id でシャード/パーティションする余地がある（Slack は Vitess で実施）」と ADR で言及するに留める。
