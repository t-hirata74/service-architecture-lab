...
variable "azs" {
  description = "利用する AZ のリスト (3-AZ HA)"
  type        = list(string)
...
variable "frontend_image" {
  ...
  type        = string
  default     = "ghcr.io/example/instagram-frontend:latest"
}

variable "backend_image" {
  description = "Django backend コンテナイメージ"
  type        = string
  default     = "ghcr.io/example/instagram-backend:latest"
}

variable "ai_worker_image" {
  description = "ai-worker (FastAPI) コンテナイメージ"
  type        = string
  default     = "ghcr.io/example/instagram-ai-worker:latest"
}
...
variable "elasticache_node_type" {
  description = "ElastiCache Redis のノードタイプ (Celery broker / 結果 backend)"
  type        = string
