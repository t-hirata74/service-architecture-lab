...
resource "aws_secretsmanager_secret" "database_url" {
  name        = "datadog/database_url"
  description = "MySQL DSN for Go dispatch"
}
...
resource "aws_secretsmanager_secret" "jwt_secret" {
  name        = "datadog/jwt_secret"
  description = "HS256 signing key for user JWT (REST + driver WS で共通, ADR 0004)"
}
...
resource "aws_secretsmanager_secret" "ai_internal_token" {
  name        = "datadog/ai_internal_token"
  description = "Shared secret between Go dispatch and ai-worker (X-Internal-Token)"
}
