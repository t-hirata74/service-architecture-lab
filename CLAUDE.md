...
| ~~`datadog`~~ → 🟡 Phase 2 | **Go** (3本目) | fan-in ingestion パイプライン (chan→worker pool→single-owner aggregator) / backpressure (load shedding) + cardinality 制御 / 固定窓 rollup 冪等 upsert / 周期評価 alert state machine (Python 候補→Go で着手) |
