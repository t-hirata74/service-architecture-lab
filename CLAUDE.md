...
| ~~`figma`~~ → 🟢 MVP 完成 | Rails 8.1 / **Ruby 4.0.5** | Server 権威 LWW-CRDT (図形キャンバス) / append-only op log + materialized state / ActionCable + Solid Cable で収束 op fan-out / cursor は ephemeral / OperationApplier + 収束不変条件 spec / DocumentChannel + rodauth JWT + ai-worker auto-layout/lint / frontend SVG canvas (client LWW) + Playwright 2 件 |
...
| ~~`linear`~~ → 🟢 MVP 完成 | **TypeScript (NestJS)** (初) | server 権威 sync log (counter 行 FOR UPDATE で gapless 採番 + 並行不変条件テスト) / bootstrap + delta sync / client SyncEngine (optimistic + offline replay + temp id remap) / reducer parity / FE/BE zod 型共有 monorepo / 素 WS push + catch-up 自己修復 / Next.js kanban + ⌘K + Playwright hstack 3 件 / E1 メンバー招待・権限 (protocol lockstep 進化 + WS kick) — figma (CRDT) と対の sync log 流派 |
