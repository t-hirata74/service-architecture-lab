...
| ~~`linear`~~ → 🟡 Phase 2 | **TypeScript (NestJS)** (初) | server 権威 sync log (per-workspace seq を counter 行 FOR UPDATE で gapless 採番) / bootstrap + delta sync / optimistic update + offline mutation queue (clientMutationId 冪等) / FE/BE zod 型共有 monorepo (npm workspaces) / 素 WS push + catch-up 自己修復 — figma (CRDT) と対の sync log 流派 |
