...
| [`uber`](uber/) | Uber 風配車プラットフォーム (Go) | **地理空間索引 (H3 hexagonal grid)** / 二者間 trip + driver state machine + compare-and-set / **per-H3-cell matcher goroutine + channel offer** (discord per-guild Hub との Go 並行 pattern 対比) | 🟡 Phase 2 完了 (go mod init + migrations + H3 wrapper + state machine 純粋関数 + 単体テスト) | [README](uber/README.md) ・ [Architecture](uber/docs/architecture.md) ・ [ADR (3)](uber/docs/adr/) |
...
| **Go** | uber (Phase 2 完了) | **H3 hexagonal grid 地理空間索引** / per-H3-cell matcher goroutine + channel offer / 二者間 trip + driver state machine + compare-and-set |
...
service-architecture-lab/
  ...
  uber/                   # Uber 風 (Go / H3 + per-cell matcher goroutine + 二者間 trip state machine / Phase 2 完了)
  docs/                   # 共通ルール (走りながら整備)
