...
| [`datadog`](datadog/) | Datadog 風メトリクス観測基盤 (Go) | **fan-in ingestion パイプライン** (ingest chan → worker pool → single-owner aggregator goroutine) / **backpressure (non-blocking drop / load shedding) + cardinality 制御** / 固定窓 rollup を MySQL 冪等 upsert / **周期評価 alert rule engine** (ok→pending→firing state machine) / Go 第3の並行型 = 多→1 fan-in (discord fan-out / uber matching と対比) | 🟡 Phase 4 完了 (ADR 0001-0004 + **fan-in ingestion パイプライン + load shedding + cardinality cap** + **alert rule engine (ok→pending→firing state machine)** + ai-worker (detect-anomaly/forecast) + internal/ai graceful degradation / go test -race 全 + pytest 9) | [README](datadog/README.md) ・ [Architecture](datadog/docs/architecture.md) ・ [ADR (4)](datadog/docs/adr/) |
