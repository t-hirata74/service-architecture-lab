...
| [`datadog`](datadog/) | Datadog 風メトリクス観測基盤 (Go) | **fan-in ingestion パイプライン** (ingest chan → worker pool → single-owner aggregator goroutine) / **backpressure (non-blocking drop / load shedding) + cardinality 制御** / 固定窓 rollup を MySQL 冪等 upsert / **周期評価 alert rule engine** (ok→pending→firing state machine) / Go 第3の並行型 = 多→1 fan-in (discord fan-out / uber matching と対比) | 🟡 Phase 3 完了 (ADR 0001-0004 + Go backend + **fan-in ingestion パイプライン (bounded chan→worker pool→single-owner aggregator→固定窓 rollup flush) + load shedding + cardinality cap + /query//stats** + go test -race 全パッケージ: load-shed/cardinality/並行 no-race/live async) | [README](datadog/README.md) ・ [Architecture](datadog/docs/architecture.md) ・ [ADR (4)](datadog/docs/adr/) |
...
| **Go** | datadog (Phase 1 設計完了) | **fan-in ingestion パイプライン** (ingest chan → worker pool → single-owner aggregator goroutine) / **backpressure (non-blocking drop / load shedding) + cardinality 制御** / 固定窓 rollup 冪等 upsert / 周期評価 alert state machine — discord fan-out / uber matching に続く **Go 第3の並行型 = 多→1 fan-in** |
...
service-architecture-lab/
  ...
  datadog/                # Datadog 風 (Go / fan-in ingestion pipeline + backpressure/cardinality + 固定窓 rollup + alert engine / Phase 1 設計フェーズ完了)
  docs/                   # 共通ルール (走りながら整備)
