...
| [`uber`](uber/) | Uber 風配車プラットフォーム (Go) | **地理空間索引 (H3 hexagonal grid)** / 二者間 trip + driver state machine + compare-and-set / **per-H3-cell matcher goroutine + channel offer** (discord per-guild Hub との Go 並行 pattern 対比) | 🟡 Phase 3 完了 (store CRUD + dispatch DB 反映層 + Matcher goroutine + CellRegistry + 100 並行 accept race 統合テスト pass) | [README](uber/README.md) ・ [Architecture](uber/docs/architecture.md) ・ [ADR (3)](uber/docs/adr/) |
