...
| **Go** | uber (Phase 4-2 完了 / backend + ai-worker MVP 動作) | **H3 hexagonal grid 地理空間索引** / per-H3-cell matcher goroutine + channel offer / 二者間 trip + driver state machine + compare-and-set / **ai-worker 同期境界 (ETA) + graceful degradation** |
...
service-architecture-lab/
  ...
  uber/                   # Uber 風 (Go / H3 + per-cell matcher goroutine + 二者間 trip state machine + ai-worker ETA 同期境界 / Phase 4-2 完了 / backend + ai-worker MVP 動作)
  docs/                   # 共通ルール (走りながら整備)
