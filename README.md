...
| [`shopify`](shopify/) | Shopify 風 EC プラットフォーム (Rails 8) | モジュラーモノリス (Rails Engine + packwerk) / マルチテナント (`shop_id` row-level scoping) / 在庫の同時減算 (条件付き UPDATE + ledger) / App プラットフォーム (Webhook at-least-once + HMAC + idempotency) | 🟡 Phase 3 完了 (Phase 2 + catalog/inventory Engine + Inventory::DeductService 条件付き UPDATE / 100 並行 thread で負在庫ゼロ実証 / RSpec 40 件通過) | [README](shopify/README.md) ・ [Architecture](shopify/docs/architecture.md) ・ [ADR (4)](shopify/docs/adr/) |
