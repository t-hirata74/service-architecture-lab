...
| [`linear`](linear/) | Linear 風 issue tracker (**TypeScript: NestJS + Next.js**) | **server 権威 sync log** (per-workspace seq を counter 行 `FOR UPDATE` で gapless 採番) / **bootstrap + delta sync** (`?since=lastSyncId` catch-up) / **optimistic update + rollback + offline mutation queue** (clientMutationId 冪等) / **FE/BE zod 型共有 monorepo** (npm workspaces) — 本リポ初 TS backend、figma (CRDT) と対の「sync log 流派」 | 🟡 Phase 4 完了 (ADR 0001-0005 + monorepo: **shared zod スキーマ/fractional indexing/reducer** + NestJS backend: `POST /mutations` 9 コマンド + **workspace 行 FOR UPDATE 採番 + 冪等台帳** + bootstrap/delta + 素 WS gateway + **並行 30 mutation gapless 不変条件テスト** + **client SyncEngine (confirmed+pending rebase / optimistic+rollback / offline replay + temp id remap / gap 自己修復)** + **reducer parity テスト (ops 畳み込み ≡ bootstrap)** + ai-worker triage/duplicates + `/ai/triage` 内部 ingress / jest e2e 30 + vitest 37 + pytest 9) | [README](linear/README.md) ・ [Architecture](linear/docs/architecture.md) ・ [ADR (5)](linear/docs/adr/) |
...
| **TypeScript (NestJS)** | linear (Phase 4 完了) | **server 権威 sync log + bootstrap/delta sync** / optimistic update + offline mutation queue / **FE/BE zod 型共有 monorepo** (npm workspaces) — 本リポ初の TS backend。「Node 版 Rails」としての NestJS 実務構成 + TS フルスタック固有の契約共有を学ぶ |
...
| ~~`linear`~~ → 着手 (TypeScript) | Linear | **server 権威 sync log (lastSyncId 全順序採番)** / bootstrap + **delta sync** / **optimistic update + offline mutation queue** (冪等 replay) / FE/BE zod 型共有 monorepo — 本リポ初 TS backend (NestJS)。figma (CRDT) と対の sync log 流派 (本リポに着手済み、Phase 1 完了) |
...
service-architecture-lab/
  ...
  linear/                 # Linear 風 (TypeScript: NestJS + Next.js / sync log + delta sync + optimistic・offline / Phase 1 設計フェーズ)
  docs/                   # 共通ルール (走りながら整備)
