...
| [`perplexity`](perplexity/) | Perplexity 風 RAG 検索 | RAG パイプライン (retrieve / extract / synthesize) / Hybrid retrieval + embedding データ管理 / SSE streaming + 三段階 degradation / 引用整合性の信頼境界 / テスト戦略 / chunk 分割戦略 | 🟡 Phase 4 完了 (SSE proxy + Frontend 動作 / RSpec 101 + pytest 70 件 / Phase 5 で auth + E2E + Terraform) | [README](perplexity/README.md) ・ [Architecture](perplexity/docs/architecture.md) ・ [ADR (6)](perplexity/docs/adr/) |
...
3 プロジェクトで踏んだ落とし穴 / 確立したパターンを `docs/` 配下に整理。次のプロジェクトで同じ罠を踏まないための土台。
...
- **[docs/coding-rules/frontend.md](docs/coding-rules/frontend.md)** — urql Provider は `useState(() => createClient())` で 1 度だけ生成 / urql は queries に GET / codegen 出力は commit して diff 検知
- **[docs/operating-patterns.md](docs/operating-patterns.md)** — 内部 trusted ingress (REST + 共有トークン) / 本番デフォルトトークン拒否ガード / urql + Playwright の cache busting
- **[docs/testing-strategy.md](docs/testing-strategy.md)** — N+1 spec / field-level auth spec / Pundit Scope spec / `with_lock` の transactional fixtures 限界
...
| **Python (Django/DRF)** | instagram (候補) | Django ORM / タイムライン生成 (push vs pull) / フォローグラフ |
| **Python (FastAPI)** | reddit (候補) | 非同期 I/O / コメントツリー DB 設計 / Hot ランキング |
| **Go** | uber (候補) | goroutine / 地理空間インデックス (S2/Geohash/H3) / 配車 state machine |
| **Go** | discord (候補) | WebSocket fan-out / ギルド単位シャーディング / プレゼンス整合性 |
...
| --- | --- | --- | --- |
| `instagram` | **Python (Django/DRF)** | Instagram | タイムライン生成戦略 (push 型 vs pull 型) / フォローグラフ DB 設計 / Django ORM の N+1 回避とインデックス |
| `reddit` | **Python (FastAPI)** | Reddit | コメントツリー DB 設計 (Adjacency List / Materialized Path / Nested Set 比較) / 投票スコア整合性 / Hot ランキング |
| `uber` | **Go** | Uber | 地理空間インデックス (S2/Geohash/H3) / goroutine + channel での並行マッチング / 配車 state machine |
| `discord` | **Go** | Discord | WebSocket fan-out / ギルド単位シャーディング / プレゼンス (ハートビート) 整合性 |
...
service-architecture-lab/
  ...
  perplexity/             # Perplexity 風 (設計フェーズ完了 / Phase 2 以降で実装)
  docs/                   # 共通ルール (走りながら整備)
    api-style.md          # REST/GraphQL 選定 + GraphQL 運用
    coding-rules/         # rails / frontend / python の規約
