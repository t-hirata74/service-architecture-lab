...
| [`discord`](discord/) | Discord 風リアルタイムチャット (Go) | ギルド単位シャーディング + 単一プロセス Hub / goroutine + channel CSP pattern / プレゼンスハートビート / WebSocket fan-out (slack Rails ActionCable との対比) | 🟡 Phase 2 REST ゲートウェイ（JWT + Guild/Channel/Message CRUD）、WS は Phase 3 | [README](discord/README.md) ・ [Architecture](discord/docs/architecture.md) ・ [ADR (4)](discord/docs/adr/) |
...
| **Go** | discord (Phase 2: REST MVP) | WebSocket fan-out / ギルド単位シャーディング / プレゼンス整合性 |
...
service-architecture-lab/
  ...
  discord/                # Discord 風 (Go / Phase 2 REST MVP)
  docs/                   # 共通ルール (走りながら整備)
