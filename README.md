...
| [`uber`](uber/) | Uber 風配車プラットフォーム (Go) | **地理空間索引 (H3 hexagonal grid)** / 二者間 trip + driver state machine + compare-and-set / **per-H3-cell matcher goroutine + channel offer** (discord per-guild Hub との Go 並行 pattern 対比) | 🔴 Phase 1 設計フェーズ (ADR 0001-0003 起こし済み、実装未着手) | [README](uber/README.md) ・ [Architecture](uber/docs/architecture.md) ・ [ADR (3)](uber/docs/adr/) |
...
| **Go** | uber (Phase 1 設計フェーズ) | **H3 hexagonal grid 地理空間索引** / per-H3-cell matcher goroutine + channel offer / 二者間 trip + driver state machine + compare-and-set |
...
service-architecture-lab/
  ...
  uber/                   # Uber 風 (Go / H3 + per-cell matcher goroutine + 二者間 trip state machine / Phase 1 設計フェーズ)
  docs/                   # 共通ルール (走りながら整備)
