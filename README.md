...
3 プロジェクトとも **backend (Rails 8) + frontend (Next.js 16) + ai-worker (Python / FastAPI) + MySQL** という同形構成。違いは **API スタイル / キュー / 認可モデル / 検索エンジン** といった技術課題ごとの選択にある。
...
| --- | --- | --- | --- |
| Frontend | Next.js 16 / React 19 / Tailwind v4 | 同左 | 同左 + **urql** + graphql-codegen |
| Backend | Rails 8 (API) / rodauth-rails / ActionCable | Rails 8 (API) / **Solid Queue** / Active Storage | Rails 8 (API) / **graphql-ruby** / Pundit / Solid Queue |
| ai-worker | FastAPI / Python 3.13 (要約 mock) | FastAPI (recommend / tags / thumbnail) | FastAPI (review / code-summary / check/run) |
| 永続化 | MySQL 8 + **Redis 7** | MySQL 8 のみ (Solid Queue/Cache 同居) | MySQL 8 のみ |
| API | **REST + OpenAPI** | REST + OpenAPI | **GraphQL** |
| 認可 | controller filter | controller + scope | **Pundit + PermissionResolver の 2 層** |
| キュー | (なし) | **Solid Queue** (DB-driven) | (Solid Queue 同梱だが未使用) |
| 検索 | (なし) | **MySQL FULLTEXT (ngram)** | (なし) |
| E2E | Playwright (chromium) 6 件 | Playwright 4 件 | Playwright 4 件 |
| CI | github-actions | 同左 | 同左 (4 ジョブ × 3 プロジェクト = 12 ジョブ並列) |
