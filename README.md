...
- **[docs/coding-rules/typescript.md](docs/coding-rules/typescript.md)** — **TS フルスタックを選ぶ判断軸 (FE/BE 契約が主役 / isomorphic ロジック / streaming BFF)** + linear で確立した規約: npm workspaces monorepo (shared は dist 配布 / client は framework 非依存 + 副作用注入) / NestJS (APP_GUARD + `@Public()` / ZodValidationPipe / 素 WS の注意点) / Prisma (`FOR UPDATE` は `$queryRaw` / BigInt 境界変換 / shadow DB) / react-hooks v6 対応 3 パターン / **toolchain の罠 早見表 9 件** / NestJS⇄Rails 対比表
...
| **TypeScript (NestJS)** | linear (MVP 完成) | **server 権威 sync log + bootstrap/delta sync** / optimistic update + offline mutation queue / **FE/BE zod 型共有 monorepo** (npm workspaces) — 本リポ初の TS backend。「Node 版 Rails」としての NestJS 実務構成 + TS フルスタック固有の契約共有 (reducer parity) を学んだ |
...
- **`linear`** は **`figma` (CRDT)** と対になる sync log 流派: 収束を client 側 merge (LWW clock) でなく **server の全順序 (seq)** で取り、client は confirmed + pending queue で optimistic / offline を扱う。ドメインは `github` (issue tracker) と重なるが技術軸が完全に別 (権限グラフ/GraphQL vs sync engine) — `slack`↔`discord` と同じ「同ドメイン別軸」比較。**本リポ初の TypeScript backend** として、FE/BE 契約共有 (zod monorepo) も固有テーマ
