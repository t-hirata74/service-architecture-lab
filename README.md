...
| [`shopify`](shopify/) | Shopify 風 EC プラットフォーム (Rails 8) | モジュラーモノリス (Rails Engine + packwerk) / マルチテナント (`shop_id` row-level scoping) / 在庫の同時減算 (条件付き UPDATE + ledger) / App プラットフォーム (Webhook at-least-once + HMAC + idempotency) | 🟡 Phase 4 完了 (Phase 3 + orders Engine + CheckoutService 同一 tx で在庫減算 / storefront REST + ai-worker FastAPI mock + Next.js 16 build pass / RSpec 56 + pytest 7 通過) | [README](shopify/README.md) ・ [Architecture](shopify/docs/architecture.md) ・ [ADR (4)](shopify/docs/adr/) |
