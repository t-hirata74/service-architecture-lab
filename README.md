...
| [`figma`](figma/) | Figma 風リアルタイム共同編集キャンバス (Rails 8 / Ruby 4) | **Server 権威 LWW-CRDT** (図形キャンバス / per-prop Lamport clock) / **append-only op log (server 採番 seq) + materialized state** / **ActionCable + Solid Cable で収束 op fan-out** (slack の Redis adapter との対比) / multiplayer cursor は ephemeral / 認証 rodauth JWT 1 経路 | 🟡 Phase 4 完了 (ADR 0001-0004 + Rails 8.1/Ruby 4.0.5 + **LWW 収束エンジン + 収束不変条件 spec** + rodauth JWT (REST+ActionCable) + documents CRUD/snapshot/catch-up + **DocumentChannel op fan-out + ephemeral cursor** + ai-worker auto-layout/lint + graceful degradation / RSpec 26 + pytest 13) | [README](figma/README.md) ・ [Architecture](figma/docs/architecture.md) ・ [ADR (4)](figma/docs/adr/) |
...
| --- | --- | --- |
| **Rails** | slack / youtube / github / perplexity / shopify / zoom / calendly | WebSocket fan-out / 状態機械 / 権限グラフ / SSE + RAG / **モジュラーモノリス + multi-tenant + webhook 配信** / **長寿命 state machine + 動的権限譲渡 + 結果 UNIQUE 冪等パイプライン** / **期間 overlap + RRULE + tz 永続化 (Ruby 4)** |
