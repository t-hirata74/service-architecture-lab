...
| **TypeScript (NestJS)** | linear (Phase 2 完了) | **server 権威 sync log + bootstrap/delta sync** / optimistic update + offline mutation queue / **FE/BE zod 型共有 monorepo** (npm workspaces) — 本リポ初の TS backend。「Node 版 Rails」としての NestJS 実務構成 + TS フルスタック固有の契約共有を学ぶ |
