...
| [`linear`](linear/) | Linear 風 issue tracker (**TypeScript: NestJS + Next.js**) | **server 権威 sync log** (per-workspace seq を counter 行 `FOR UPDATE` で gapless 採番) / **bootstrap + delta sync** (`?since=lastSyncId` catch-up) / **optimistic update + rollback + offline mutation queue** (clientMutationId 冪等) / **FE/BE zod 型共有 monorepo** (npm workspaces) — 本リポ初 TS backend、figma (CRDT) と対の「sync log 流派」 | 🟡 Phase 3 完了 (ADR 0001-0005 + **npm workspaces monorepo + shared zod スキーマ/fractional indexing** + NestJS backend: Prisma 11 テーブル + JWT 認証 + `POST /mutations` 9 コマンド + **workspace 行 FOR UPDATE 採番 + clientMutationId 冪等台帳** + **bootstrap/delta + 素 WS gateway (hello + op push + heartbeat)** + **並行 30 mutation gapless 不変条件テスト** / jest e2e 26 + vitest 14 + eslint green) | [README](linear/README.md) ・ [Architecture](linear/docs/architecture.md) ・ [ADR (5)](linear/docs/adr/) |
...
| **TypeScript (NestJS)** | linear (Phase 3 完了) | **server 権威 sync log + bootstrap/delta sync** / optimistic update + offline mutation queue / **FE/BE zod 型共有 monorepo** (npm workspaces) — 本リポ初の TS backend。「Node 版 Rails」としての NestJS 実務構成 + TS フルスタック固有の契約共有を学ぶ |
