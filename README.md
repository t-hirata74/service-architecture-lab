...
| [`shopify`](shopify/) | Shopify 風 EC プラットフォーム (Rails 8) | モジュラーモノリス (Rails Engine + packwerk) / マルチテナント (`shop_id` row-level scoping) / 在庫の同時減算 (条件付き UPDATE + ledger) / App プラットフォーム (Webhook at-least-once + HMAC + idempotency) | 🟢 MVP 完成 (Phase 1-5 完了 / Rails 5 Engine + packwerk 0 violation / orders → apps の dependency inversion を ActiveSupport::Notifications で実装 / RSpec 71 + pytest 7 + Terraform validate / CI 5 ジョブ) | [README](shopify/README.md) ・ [Architecture](shopify/docs/architecture.md) ・ [ADR (4)](shopify/docs/adr/) |
...
- **[docs/operating-patterns.md](docs/operating-patterns.md)** — 内部 trusted ingress (REST + 共有トークン) / SSE 三段階 degradation + **graceful degradation で 4xx 吸収 (httpx 罠)** / **fan-out on write** + soft delete / **denormalized counter + recount management command** / **single-process Hub goroutine + CSP fan-out (discord)** / **Materialized Path comment tree (reddit)** / **truth + denormalized + 相対加算 + reconcile (reddit)** / **APScheduler driven batch + single-instance constraint (reddit)**
