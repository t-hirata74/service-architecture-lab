...
# ALB
#
# Shopify は WebSocket を持たないので普通の REST 用 ALB。
...
resource "aws_lb" "main" {
  ...
  tags = { Name = "shopify-alb" }
}

resource "aws_lb_target_group" "frontend" {
  name        = "shopify-front-tg"
  port        = 3000
  protocol    = "HTTP"
  ...
  tags = { Name = "shopify-front-tg" }
}

resource "aws_lb_target_group" "backend" {
  name        = "shopify-back-tg"
  port        = 8000
  protocol    = "HTTP"
  vpc_id      = aws_vpc.main.id
  target_type = "ip"
