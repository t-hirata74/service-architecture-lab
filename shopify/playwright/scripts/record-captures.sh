...
# `captures/<test-name>.gif` に置く (ADR 0001-0004 で 1 本ずつ)。
#
# 使い方:
#   cd shopify/playwright && npm run capture
#   PLAYBACK_RATE=1.5 npm run capture
#
# 必要: ffmpeg + 全サービス起動可能 (mysql / Rails / mock_receiver / Next)
...
capture_name_for() {
  case "$1" in
    *01-modular-monolith*) echo "01-modular-monolith" ;;
    *02-multi-tenancy*)    echo "02-multi-tenancy" ;;
    *03-inventory-race*)   echo "03-inventory-race" ;;
    *04-webhook-delivery*) echo "04-webhook-delivery" ;;
